...
const githubRedirectUri =
  (process.env.REACT_APP_SITE_URL || "http://localhost:3000") + "/github-callback";

const handleSocialLogin = (provider) => {
  console.debug("Social login initiated:", { provider, apiUrl });
  if (provider === "google") {
...
const LoginPage = () => {
  ...
  const handleGoogleLoginSuccess = async (credentialResponse) => {
    try {
      console.debug("Google login attempt:", {
  ...
  const imageSrc =
    currentTheme === "blue" ? "/wasteimage-blue.png" : "/wasteimage.png";
