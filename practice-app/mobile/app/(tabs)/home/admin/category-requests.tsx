...
export default function CategoryRequestsScreen() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch category requests, status:', response.status);
        setRequests([]);
        setFilteredRequests([]);
      }
    } catch (error) {
  ...
  const handleApprove = async (requestId: number) => {
    try {
      const response = await tokenManager.authenticatedFetch(API_ENDPOINTS.WASTE.ADMIN.APPROVE_CATEGORY_REQUEST(requestId), { method: 'POST' });
      if (response.ok) {
        const updatedRequests = requests.map(req => req.id === requestId ? { ...req, status: 'approved' as const } : req);
        setRequests(updatedRequests);
        setFilteredRequests(filteredRequests.map(req => req.id === requestId ? { ...req, status: 'approved' as const } : req));
  ...
  const handleReject = async (requestId: number) => {
    try {
      const response = await tokenManager.authenticatedFetch(API_ENDPOINTS.WASTE.ADMIN.REJECT_CATEGORY_REQUEST(requestId), { method: 'POST' });
      if (response.ok) {
        const updatedRequests = requests.map(req => req.id === requestId ? { ...req, status: 'rejected' as const } : req);
        setRequests(updatedRequests);
        setFilteredRequests(filteredRequests.map(req => req.id === requestId ? { ...req, status: 'rejected' as const } : req));
          ...
          {item.user && (
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>
                {typeof item.user === 'object' && item.user.username ? item.user.username :
                 typeof item.user === 'number' ? `User #${item.user}` : ''}
              </Text>
            </View>
          )}
        </View>
  ...
  const renderHeader = useMemo(() => (
    <View style={styles.header}>
      <Text style={styles.headerTitle}>Category Requests</Text>
      <Text style={styles.headerSubtitle}>Review and manage user requests for new waste categories</Text>
    </View>
  ), []);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.searchContainer}>
        ...
        <TextInput style={styles.searchInput} placeholder="Search requests..." value={searchQuery} onChangeText={handleSearch} />
        ...
        ListEmptyComponent={() => (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              {searchQuery.length > 0 ? "No requests match your search" : "No category requests found"}
            </Text>
          </View>
        )}
      />
...
const styles = StyleSheet.create({
  ...
  searchContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', borderRadius: 12, margin: 16, paddingHorizontal: 16, paddingVertical: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
  searchIcon: { marginRight: 8 },
  searchInput: { flex: 1, height: 40, fontSize: 16 },
  listContainer: { padding: 16, paddingTop: 0 },
  requestCard: { backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
  requestHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  requestName: { fontSize: 18, fontWeight: '600', flex: 1, marginRight: 8 },
  statusBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, backgroundColor: '#E0E0E0' },
  pendingBadge: { backgroundColor: '#F57C00' },
  approvedBadge: { backgroundColor: '#2E7D32' },
  rejectedBadge: { backgroundColor: '#D32F2F' },
  statusBadgeText: { fontSize: 12, color: '#FFFFFF', fontWeight: '500' },
  requestDescription: { fontSize: 14, color: '#333', marginBottom: 16 },
  detailsContainer: { backgroundColor: '#F5F5F5', padding: 12, borderRadius: 8, marginBottom: 16 },
  detailRow: { flexDirection: 'row', marginBottom: 8 },
  detailLabel: { fontSize: 14, fontWeight: '500', width: 100 },
