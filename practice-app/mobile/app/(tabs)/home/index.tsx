...
const PostImage = ({ source, width }: { source: ImageSourcePropType, width: number }) => {
  ...
  useEffect(() => {
    ...
    if (typeof source === "number") {
      ...
      Image.getSize(source, (w, h) => setHeight(width * (h / w)), (err) => console.error(err));
    }
  }, [source, width]);
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.scrollContent}>
        ...
        {activeGoals.length > 0 ? (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.horizontalScrollContent}>
            {activeGoals.map(goal => (
              <TouchableOpacity activeOpacity={1} key={goal.id} style={styles.goalScrollItem} onPress={() => router.push({ pathname: "/goals/[id]", params: { id: goal.id } })}>
                ...
                <View>
                  <View style={styles.progressContainer}>
                    ...
                    <Text style={styles.progressText}>{goal.progress.toFixed(1)}/{goal.target.toFixed(1)} kg</Text>
                  </View>
        ...
        {dummyPosts.map(post => (
          <View key={post.id} style={styles.postCard}>
            ...
            {post.postImage && <PostImage source={post.postImage} width={width - 32} />}
